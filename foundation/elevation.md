...
| Токен | Назначение |
|---|---|
| `shadow/ambient` | Лёгкая тень для raised или scrolled surface |
| `shadow/overlay` | Усиленная тень для overlays и floating surfaces |

Примечание о текущем состоянии: `shadow/ambient` и `shadow/overlay` сейчас являются цвето-семантическими токенами. Новые specs должны описывать уровень elevation, а shadow token использовать как реализационное значение.
...
|---|---|---|---|---|---|
| `elevation/none` | В потоке страницы | `surface/base` | По необходимости | Нет | Формы, таблицы, обычные секции |
| `elevation/outlined` | Отделённая поверхность | `surface/base` | `border/default` | Нет | Card outlined, panels, table container |
| `elevation/raised` | Лёгкий подъём | `surface/base` | Optional | `shadow/ambient` | Scrolled Top Bar, clickable card hover |
| `elevation/floating` | Floating layer над страницей | `surface/base` | `border/default` | `shadow/overlay` | Dropdown, Popover, Tooltip, Search dropdown |
| `elevation/overlay` | Панель поверх контента | `surface/base` | `border/default` | `shadow/overlay` | Drawer, Command palette, Notification Center |
| `elevation/modal` | Блокирующий слой | `surface/base` | `border/default` | `shadow/overlay` + scrim | Modal/Dialog |
| `elevation/toast` | Временный notification layer | `surface/inverse` | Optional | `shadow/overlay` | Toast/Snackbar |
...
Текущие specs часто используют `shadow/overlay` как overlay color (`--modal-overlay`, `--drawer-overlay`). В следующей итерации лучше ввести отдельный token `overlay/scrim`.
...
| Использовать `shadow/ambient` для лёгкого подъёма | Делать hover elevation с layout shift |
| Использовать `shadow/overlay` для floating/overlay | Использовать один shadow для всех уровней |

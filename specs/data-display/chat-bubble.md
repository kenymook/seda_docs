...
> **Figma** · [Chat Bubble](https://www.figma.com/design/Su1jWqKc9TkD1R8f7wHOQU/SEDA-AI--v0.2.0?node-id=7063-1570)
...
| `content` | Да | Текст, изображение, файл или audio content в текущем Figma contract; code/rich описаны как roadmap content model. |
...
| `deliveryStatus` | Условно | Figma variants: `sent`, `delivered`, `read`, `error`; `sending`, `edited`, `streaming` остаются product/code states до отдельного variant pass. |
...
| `code` | Код или технический output. | Roadmap: пока не является Figma variant; нужен отдельный code block/content contract. |
| `rich` | Смешанный контент. | Roadmap: пока не является Figma variant; нужна явная content model. |
...
|---|---|---|
| `s` | Меньше spacing и metadata. | Support panel, side panel, dense logs. |
| `m` | Default spacing. | Основной chat UI. |
| `l` | Больше воздуха и ширины. | Detail view, rich AI response, file/audio content. |
| `xl` | Максимальная ширина и читаемость. | Expanded chat, review mode, long AI responses. |

### Правила размеров

- `m` используется по умолчанию.
...
| `sending` | Сообщение отправляется. | Roadmap/code state: нужен текст или accessible status до добавления Figma variant. |
...
| Read status foreground | `chat-bubble/status/read` | `status/info/text` |
| Error status foreground | `chat-bubble/status/error` | `status/danger/text` |

### Token gaps

- Нет component tokens для AI/system surface/foreground/border/icon, outgoing/incoming hover surfaces, secondary foreground, sent/delivered status colors, radius, tail, spacing, max width, attachment surface, reaction surface, action icons и streaming indicator.
...
|---|---|---|
| Sender | `sender` | Figma variants: `incoming`, `outgoing`, `ai`, `system`. |
| Content type | `contentType` | Figma variants: `text`, `image`, `file`, `audio`; `code` и `rich` пока roadmap/code-level content models. |
...
| Delivery status | `deliveryStatus` | Figma variants: `sent`, `delivered`, `read`, `error`; `sending`, `edited`, `streaming` пока code/product states. |
...
| AI label | `aiLabel` | Обязателен для `sender="ai"`, если source нужно различать. |

### Contract rules

- `content` обязателен.
- `sender` и `contentType` используют только documented values.
- `deliveryStatus` требует accessible text.
- `sender="ai"` не должен подразумевать human authorship.
...
| Scenario | Usage |
|---|---|
| Ответ оператора поддержки | `sender="incoming"`, author visible, timestamp available. |
| Сообщение текущего пользователя | `sender="outgoing"`, `deliveryStatus="read"` с accessible label. |
| Ответ AI-ассистента | `sender="ai"`, AI label, copy/feedback nested actions. |

...
| `error` | Ошибка валидации | `border` → `status/error/border`, helper красный |
| `warning` | Предупреждение | `border` → `status/warning/border` |
| `success` | Успешная валидация | `border` → `status/success/border` |
| `read-only` | Только чтение | Фон `surface/default`, cursor `default` |
...
### Character count
Формат `[текущее] / [максимальное]`. При ≥ 80% — `status/warning/text`. При превышении — `status/error/text`.
...
|---|---|---|---|
| `--input-bg` | Фон default | `surface/default` | `surface/default` |
| `--input-bg-hover` | Фон hover | `surface/hover` | `surface/hover` |
...
| `--input-border-error` | Граница error | `status/error/border` | `status/error/border` |
...
| `--input-helper-error` | Helper error | `status/error/text` | `status/error/text` |
...
| `--input-affix-bg` | Фон зоны affix-текста | `container/default` | `container/default` |

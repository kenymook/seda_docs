...
| `error` | Ошибка валидации | `border` → `status/error/border` |
...
### Character count
Аналогично Text Field: `[текущее] / [максимальное]`. При ≥ 80% — `status/warning/text`. При превышении — `status/error/text`.
...
|---|---|---|---|
| `--textarea-bg` | Фон default | `surface/default` | `surface/default` |
...
| `--textarea-border-error` | Граница error | `status/error/border` | `status/error/border` |
...
| `--textarea-helper-error` | Helper error | `status/error/text` | `status/error/text` |

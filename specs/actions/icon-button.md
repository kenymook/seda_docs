...
|---|---|---|---|---|
| `small` | 24×24px | `icon/14` | 6px | Строки таблиц, компактные тулбары |
| `medium` | 32×32px | `icon/16` | 8px | Карточки, панели — дефолт |
| `large` | 40×40px | `icon/18` | 10px | Акцентные действия |
| `extraLarge` | 48×48px | `icon/20` | 12px | Hero, мобильные интерфейсы |
...
### ghost
Нет границы и фона в default. При hover появляется `container/default`. Самый частый вариант в тулбарах.

### destruction
Красный фон `status/error/surface`. Иконка — предпочтительно корзина или крест. Всегда с подтверждением.
...
| Component token | Роль | Semantic (Light) | Semantic (Dark) |
|---|---|---|---|
| `--icon-btn-bg-hover` | Фон hover | `container/default` | `container/default` |
| `--icon-btn-bg-pressed` | Фон active | `container/pressed` | `container/pressed` |
...
| Component token | Роль | Semantic (Light) | Semantic (Dark) |
|---|---|---|---|
| `--icon-btn-destruction-bg` | Фон default | `status/error/surface` | `status/error/surface` |
| `--icon-btn-destruction-icon` | Цвет иконки | `status/error/text` | `status/error/text` |
